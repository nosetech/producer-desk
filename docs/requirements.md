...
- 案A（GitHub issueコメント経由）でMVPを開始する。
  - ダッシュボード上のワンタップ操作（[2-7](#2-7-承認却下自由記述による指示出しの要件)）は、内部的にissueへのコメント投稿として実装する。
...
- ワンタップ承認・却下に加え、ダッシュボードのテキストボックスから**自由記述のプロンプトで指示を出せる**ようにする（[2-7](#2-7-承認却下自由記述による指示出しの要件)）。対象は以下の両方とする。
...
- ダッシュボード（判断待ち一覧／最近の活動ログ／利用量・リミットモニター、ワンタップ承認・却下、自由記述による指示入力欄）
