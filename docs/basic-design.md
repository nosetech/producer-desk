...
- **ヘルスチェック**: ワンショット実行のため常時稼働の監視は不要。プロセスの終了コード（0=正常終了、非0=異常終了）を確認する。異常終了時はissueに終了コード・ログパスを記載したコメントを投稿し、`needs-human-decision` ラベルに遷移させて人間の確認を促す（正常終了時のラベル遷移は、Agent Runner自身が実行中に `gh` コマンドで行う自己付与を第一とする。[1章](#1-データモデル状態遷移設計)参照）。
