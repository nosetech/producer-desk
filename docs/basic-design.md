...
- `DesignSync`が権限不足等で使えない場合のフォールバックは、(1)ブラウザ操作ツールでデザインのチャット欄を開き要約させず生のソースコードをそのまま出力するよう依頼する、(2)それも使えない場合は`needs-human-decision`として人間の確認を仰ぐ、の優先順で行う。いずれの方法で値を取得した場合も、実装後はブラウザ操作ツール（`mcp__claude-in-chrome__*`）で完成品とデザインのプレビューを並べて最終的な見た目の一致を確認する。
- `--chrome` フラグにより、Claude in Chrome連携を明示的に有効化する。`claude -p`（非対話モード）はこの連携がデフォルト無効で、フラグなしでは`mcp__claude-in-chrome__*`ツール自体が存在せず、上記のブラウザ確認・フォールバックが機能しない（issue #33の追加原因調査で判明）。
  - **運用上の前提**: `DesignSync`の利用には`claude.ai`ログインへのデザインシステムアクセス権限が必要（未付与の場合はユーザーに`/design-login`の実行を依頼する）。ブラウザ操作ツールのフォールバックが機能するには、Agent Runnerを実行するホスト上でChromeが起動しており、`claude-in-chrome`拡張がペアリング済みで、`claude.ai`にログイン済みである必要がある（就寝中などプロデューサーが操作しない時間帯にAgent Runnerが動く想定のため、事前にログイン状態を維持しておくこと）。いずれも利用できない場合、Agent Runnerはその旨を実行結果コメントに明記し、`needs-human-decision`として人間の確認を仰ぐ。
