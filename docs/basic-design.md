...
- **MVPでは同一LAN内からのアクセスのみ**を前提とする。ダッシュボード・オーケストレータの待受プロセスは、0.0.0.0ではなく**LANインターフェースのIPにのみbind**する（例: Next.jsの起動オプションで `--hostname <lan-ip>` を指定）。これにより、同一LANの外からは到達不能にする。
- **将来拡張**: 外出先からのアクセスに対応する際は、bind先をTailscaleインターフェースIPに切り替える（`--hostname <tailscale-ip>`）。別issueで対応する（[要件定義書 4-2](./requirements.md#4-2-将来拡張とする範囲)参照）。
