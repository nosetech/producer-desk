...
対象issueが`status:in-review`の状態で「承認」した場合はこのフローに乗らず、[2-3「既存issueへの指示」](#既存issueへの指示)記載の通り紐づくPRのsquash merge・issueクローズ・headブランチ削除のみを行う特別処理となる（コメント投稿・ラベル遷移・Agent Runnerディスパッチはいずれも行わない）。この場合も上記6・7の同期更新・再取得完了待ちは同様に適用される。
