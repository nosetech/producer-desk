...
- **プロジェクトごとのデフォルト設定**: [2-1](#2-1-対象リポジトリ一覧の管理)の`config/projects.yaml`に、プロジェクトエントリごとの実行手段設定（`execution_mode`: 既定は`claude_code`、`litellm_proxy`選択時はあわせて`litellm_model`にLiteLLM Proxy側のモデルエイリアス指定が必須。`orchestrator/orchestrator/config.py`の`Project`）を追加する（issue #176）。
  - **永続化API（issue #176、#175が依存）**: `PATCH /api/projects/{repo}/settings`（[2-2](#2-2-データ取得仕様ポーリング)・[2-3](#2-3-指示出しapi内部api)と同じ内部APIサーバー）。リクエストボディ`{"execution_mode": "claude_code" | "litellm_proxy", "litellm_model": "string（litellm_proxy時のみ必須）"}`を受け取り、`config.update_project_execution_settings`で`config/projects.yaml`の対象エントリを書き換えたのち、レスポンスとして更新後の設定を`{"repo": ..., "execution_mode": ..., "litellm_model": ...}`で返す（200）。未登録リポジトリは404、バリデーションエラー（不正な`execution_mode`・`litellm_proxy`時の`litellm_model`欠落）は400。オーケストレータ再起動を待たず次回ディスパッチから反映されるよう、`server.py`はサーバー起動時に読み込み済みの`Project`インスタンス自体を書き換える（`projects`/`projects_by_repo`と`main.py`の`_make_dispatch_fn`が参照するインスタンスは同一オブジェクトのため、この変更は両方に反映される）。ダッシュボードのプロジェクト設定UI（issue #175）はこのAPIを呼び出す想定。
