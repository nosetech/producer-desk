...
- **AIへの指示導線**: 案A（GitHub issueコメント経由）でMVPを構築する。ダッシュボード上のワンタップ承認・却下、および自由記述のテキストボックスによる指示（[要件定義書 2-7](./requirements.md#2-7-承認却下自由記述による指示出しの要件)）は、いずれも内部的にはissueへのコメント投稿として実装する。
